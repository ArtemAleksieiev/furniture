...
$FROM = 'your Comp. <mail@mail.ru>';  //  e-mail отправителя (иногда требуется указать разрешенный настройками e-mail)
$REPLY = 'no@mail.ru';   //  e-mail для ответа 
...
}elseif(isset($_POST['name'])) { // Если поле action не указано, то обрабатываем только поля по умолчанию
    sendmail("Вопрос с сайта", render(
                    '<div><b>Имя:</b> '.stripinput($_POST['name']).'</div>'.
                    '<div><b>Почта:</b> '.stripinput($_POST['email']).'</div>'.
                    '<div><b>Содежание вопроса:</b> '.stripinput($_POST['intro']).'</div>'
